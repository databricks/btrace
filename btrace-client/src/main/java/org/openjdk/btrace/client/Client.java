...
public class Client {
  ...
  void reconnect(String host, String resumeProbe, CommandListener listener, String[] command) throws IOException {
    if (sock != null) {
      throw new IllegalStateException();
    }
    try {
                ...
                if (statusCommand.getFlag() == ReconnectCommand.STATUS_FLAG) {
                  if (statusCommand.isSuccess()) {
                    ...
                    if (probeCommand != null) {
                      DebugSupport.info("Executing remote command '" + probeCommand + "'" + (probeCommandArg != null ? "(" + probeCommandArg + ")" : ""));
                      switch (probeCommand) {
                        case "exit": {
                          sendExit();
                          break;
